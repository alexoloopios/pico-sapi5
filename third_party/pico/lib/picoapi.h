...
#ifdef _WIN32
/* Local change for the SAPI 5 port: the engine is linked into the SAPI DLL as a
   static library, and with the unconditional dllexport below every pico_*
   symbol would be re-exported from it alongside the four COM entry points.
   Building with PICO_STATIC keeps the export table to what SAPI asks for. */
#  ifdef PICO_STATIC
