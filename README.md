...
You can hear them without installing anything: [samples/](samples/) holds a
rendering of each voice, plus the rate and pitch extremes.

## Installing

Download `PicoSAPI5-1.0.0-setup.exe` from the
[Releases](https://github.com/alexoloopios/pico-sapi5/releases) page, run it, and
...
## What is in the repository, and what is in a release

The Pico engine sources and all six voices' lingware are committed, so a clone
speaks without downloading or building anything first. That costs about 8 MB of
clone, which is a deliberate trade — the rendered samples are another 1.4 MB.

Built output is not committed. The two engine DLLs and the installer are
regenerated by `tools\build_all.ps1` in about twenty seconds, so `dist\` and
`build\` are ignored, and the installer ships as a release asset instead.

To cut a release: run `.\tools\build_all.ps1 -Clean -Installer` and attach
`dist\PicoSAPI5-<version>-setup.exe` to the tag. The build prints that file's
size and SHA-256 when it finishes, which is a quick way to be sure you are
uploading the installer you just built rather than one left in `dist\`.
