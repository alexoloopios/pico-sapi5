...
/* Local change for the SAPI 5 port. The narrow fopen() below resolves its path
   through the active ANSI code page, so a resource under, say, a user profile
   whose name is not representable there cannot be opened at all. On Windows the
   caller passes the name as UTF-8 and it is widened here, which makes any path
   the filesystem accepts work. */
static picopal_File picopal_fopen_utf8_win (picopal_char filename[], const wchar_t * wmode)
{
    wchar_t * wfilename;
    int wlen;
    picopal_File res;

    wlen = MultiByteToWideChar(CP_UTF8, 0, (const char *)filename, -1, NULL, 0);
    if (wlen <= 0) {
        return (picopal_File) NULL;
    }
    wfilename = (wchar_t *) malloc((size_t)wlen * sizeof(wchar_t));
    if (wfilename == NULL) {
        return (picopal_File) NULL;
    }
    if (MultiByteToWideChar(CP_UTF8, 0, (const char *)filename, -1, wfilename, wlen) <= 0) {
        free(wfilename);
        return (picopal_File) NULL;
    }
    res = (picopal_File) _wfopen(wfilename, wmode);
    free(wfilename);
    return res;
}
...
picopal_File picopal_fopen (picopal_char filename[], picopal_access_mode mode)
{
    ...
    case PICOPAL_TEXT_READ :
        return picopal_fopen_utf8_win(filename, L"r");
    case PICOPAL_TEXT_WRITE :
        return picopal_fopen_utf8_win(filename, L"w");
    case PICOPAL_BINARY_READ :
        return picopal_fopen_utf8_win(filename, L"rb");
    case PICOPAL_BINARY_WRITE :
        return picopal_fopen_utf8_win(filename, L"wb");
    default :
        return (picopal_File) NULL;
    }
}
...
#endif /* PICO_PLATFORM == PICO_Windows */
